...
function App() {
  ...
  const getData = async () => {
    const educationResponse = await fetch('http://localhost/api/education')
    const educationData = await educationResponse.json()
    setEducation(educationData)

    const workResponse = await fetch('http://localhost/api/work')
    const workData = await workResponse.json()
    setWork(workData)

    const portfolioResponse = await fetch('http://localhost/api/portfolio')
  ...
  return (
    <>
      ...
      <div className='md:h-screen pt-10' ref={homeRef}>
        ...
        <div className='flex md:flex-row flex-col space-between md:w-3/4 m-auto py-5 border rounded-sm px-3 shadow'>
          <img alt='profile' className='rounded-full w-64 h-64 md:mx-8 mx-auto' src='./profile_picture.jpg' />
          <div>
